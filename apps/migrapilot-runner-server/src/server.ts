...
app.get("/health", (_request, response) => {
  response.json({
    ...
    runner: "server",
    port,
    inventoryPath: process.env.MIGRAPILOT_INVENTORY_PATH ?? "/etc/migrapilot/inventory.json",
    journalPath: process.env.MIGRAPILOT_JOURNAL_PATH ?? ".migrapilot/journal.ndjson"
  });
});
...
app.post("/execute", async (request, response) => {
  ...
  const context = {
    environment: (input.environment as "dev" | "stage" | "staging" | "prod" | "test" | undefined) ?? "prod",
    operator: (input.operator as Record<string, unknown> | undefined) ?? {},
    runner: {
      runnerId: "server-runner-1",
      runnerType: "server" as const
    },
  ...
  console.log(`[runner-server] tool=${body.toolName} runId=${context.runId ?? "none"}`);
