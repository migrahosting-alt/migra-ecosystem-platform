...
class FakeProcesses implements AgentRecipeProcessManagerLike {
  ...
  async execute(runId: string, value: AgentRecipePlan, hooks: { onSpawned(identity: AgentContainmentIdentity): void | Promise<void> }): Promise<AgentRecipeExecutionOutcome> {
...
async function seedPending(journal: AgentRunJournal, runId: string, workspace: string, at = Date.now()): Promise<void> {
...
async function rejectRun(journal: AgentRunJournal, runId: string, at = Date.now()): Promise<void> {
  await journal.transition({ runId, expectedState: 'AWAITING_APPROVAL', nextState: 'REJECTED', at, eventType: 'approval.rejected', source: 'APPROVAL', reason: 'HUMAN_REJECTED', approvalDecisionAt: at, terminalAt: at, failureCode: 'REJECTED', approvalLifecycle: 'REJECTED', approvalDecisionType: 'REJECTED', recoveryClass: 'REPROPOSAL_ALLOWED', recoveryEligible: true, recoveryReason: 'REJECTED_FRESH_PROPOSAL_ALLOWED' });
}
async function cancelRun(journal: AgentRunJournal, runId: string, at = Date.now()): Promise<void> {
  await journal.transition({ runId, expectedState: 'AWAITING_APPROVAL', nextState: 'CANCELLED', at, eventType: 'cancellation.requested', source: 'API', reason: 'CANCELLED_BEFORE_SPAWN', terminalAt: at, failureCode: 'CANCELLED_BEFORE_SPAWN', approvalLifecycle: 'INVALIDATED', approvalInvalidationReason: 'CANCELLED_BEFORE_SPAWN', recoveryClass: 'REPROPOSAL_ALLOWED', recoveryEligible: true, recoveryReason: 'CANCELLED_FRESH_PROPOSAL_ALLOWED' });
}
async function expireRun(journal: AgentRunJournal, runId: string, at = Date.now()): Promise<void> {
  await journal.transition({ runId, expectedState: 'AWAITING_APPROVAL', nextState: 'EXPIRED', at, eventType: 'approval.expired', source: 'CLEANUP', reason: 'APPROVAL_TTL_EXPIRED', terminalAt: at, failureCode: 'EXPIRED', approvalLifecycle: 'EXPIRED', approvalInvalidationReason: 'APPROVAL_TTL_EXPIRED', recoveryClass: 'REPROPOSAL_ALLOWED', recoveryEligible: true, recoveryReason: 'EXPIRED_FRESH_PROPOSAL_ALLOWED' });
}
async function restartLoseApproval(journal: AgentRunJournal, runId: string, at = Date.now()): Promise<void> {
  await journal.transition({ runId, expectedState: 'AWAITING_APPROVAL', nextState: 'EXPIRED', at, eventType: 'approval.lost_on_restart', source: 'RECONCILIATION', reason: 'RESTART_AUTHORIZATION_LOST', terminalAt: at, failureCode: 'RESTART_AUTHORIZATION_LOST', approvalLifecycle: 'LOST_ON_RESTART', recoveryClass: 'REPROPOSAL_REQUIRED', recoveryEligible: true, recoveryReason: 'RESTART_AUTHORIZATION_LOST' });
}
async function completeRun(journal: AgentRunJournal, persistence: MemoryAgentRunJournalPersistence, runId: string, at = Date.now()): Promise<void> {
  await journal.transition({ runId, expectedState: 'AWAITING_APPROVAL', nextState: 'APPROVED', at, eventType: 'approval.approved', source: 'APPROVAL', reason: 'HUMAN_APPROVED', approvalDecisionAt: at, approvalLifecycle: 'APPROVED', approvalDecisionType: 'APPROVED' });
  await journal.transition({ runId, expectedState: 'APPROVED', nextState: 'EXECUTING', at: at + 1, eventType: 'execution.start_requested', source: 'EXECUTION', reason: 'APPROVED_EXECUTION_START', executionStartedAt: at + 1, approvalLifecycle: 'CONSUMED' });
  const identity = containmentIdentityForPlan(runId, plan('/workspace', runId));
  await journal.transition({ runId, expectedState: 'EXECUTING', nextState: 'EXECUTING', at: at + 2, eventType: 'execution.spawned', source: 'EXECUTION', reason: 'CONTAINMENT_STARTED', containmentUnit: identity.unit, containmentBinding: identity.binding });
  await journal.transition({ runId, expectedState: 'EXECUTING', nextState: 'COMPLETED', at: at + 3, eventType: 'execution.completed', source: 'EXECUTION', reason: 'PROCESS_EXITED', terminalAt: at + 3, exitCode: 0 });
  assert.equal(persistence.runs.get(runId)?.state, 'COMPLETED');
}

async function provenanceOf(journal: AgentRunJournal, runId: string) {
  const run = (await journal.loadRun(runId))!;
  return validateRecoverySourceProvenance({ run, events: await journal.events(runId), workspaceIdentity: 'workspace-id', allowedRecipes: ['git.status', 'git.diff'], now: Date.now() });
}

async function summaryOf(history: AgentRunHistoryService, runId: string, workspace: string) {
  const detail = (await history.detail(runId, context(workspace)));
...
// ---------------------------------------------------------------------------
const MATRIX: { name: string; seed: (h: ReturnType<typeof harness>, runId: string, workspace: string) => void | Promise<void>; recoveryClass: AgentModeRecoveryClass; eligible: boolean }[] = [
...
for (const entry of MATRIX) {
  test(`policy matrix: ${entry.name} is trusted with ${entry.recoveryClass} (eligible=${entry.eligible})`, async () => {
    const workspace = root();
    const h = harness();
    const runId = 'agentcmd_matrix';
    await seedPending(h.journal, runId, workspace);
    await entry.seed(h, runId, workspace);

    const provenance = (await provenanceOf(h.journal, runId));
    ...
    const summary = (await summaryOf(h.history, runId, workspace));
...
test('non-recoverable-by-policy reports a dedicated code, not an integrity failure', async () => {
  const workspace = root();
  const h = harness();
  await seedPending(h.journal, 'agentcmd_done', workspace);
  await completeRun(h.journal, h.persistence, 'agentcmd_done');
  const provenance = (await provenanceOf(h.journal, 'agentcmd_done'));
...
test('rejected source before successor linkage is trusted and eligible', async () => {
  const workspace = root();
  const h = harness();
  await seedPending(h.journal, 'agentcmd_src', workspace);
  await rejectRun(h.journal, 'agentcmd_src');
  const provenance = (await provenanceOf(h.journal, 'agentcmd_src'));
  assert.equal(provenance.trusted, true);
  assert.equal(provenance.eligible, true);
  assert.equal(provenance.recoveryClass, 'REPROPOSAL_ALLOWED');
  const status = (await h.service.getRunRecoveryStatus('agentcmd_src', context(workspace)));
...
test('rejected source after successor linkage is trusted, ineligible, and records the successor', async () => {
  ...
  const summary = (await summaryOf(h.history, 'agentcmd_src', workspace));
  assert.equal(summary.integrity, 'TRUSTED');
  assert.deepEqual(summary.integrityIssues, []);
  ...
  const stored = (await h.journal.loadRun('agentcmd_src'))!;
...
test('a forged event chain is genuinely untrusted with an integrity code', async () => {
  const workspace = root();
  const h = harness();
  await seedPending(h.journal, 'agentcmd_forged', workspace);
  await rejectRun(h.journal, 'agentcmd_forged');
  ...
  const provenance = (await provenanceOf(h.journal, 'agentcmd_forged'));
  assert.equal(provenance.trusted, false);
  assert.equal(provenance.code, 'SOURCE_INTEGRITY_FAILED');
  assert.equal(provenance.eligible, false);

  const summary = (await summaryOf(h.history, 'agentcmd_forged', workspace));
...
test('a duplicated event sequence is untrusted', async () => {
  const workspace = root();
  const h = harness();
  await seedPending(h.journal, 'agentcmd_gap', workspace);
  await rejectRun(h.journal, 'agentcmd_gap');
  const events = h.persistence.events.get('agentcmd_gap')!;
  events.push({ ...events[events.length - 1]!, seq: events.length + 5 });

  const summary = (await summaryOf(h.history, 'agentcmd_gap', workspace));
  assert.equal(summary.integrity, 'UNTRUSTED');
});
...
test('legacy rows with stale recovery_eligible=1 plus successor linkage normalize to trusted and ineligible', async () => {
  const workspace = root();
  const h = harness();
  await seedPending(h.journal, 'agentcmd_legacy', workspace);
  await rejectRun(h.journal, 'agentcmd_legacy');
  ...
  const summary = (await summaryOf(h.history, 'agentcmd_legacy', workspace));
...
test('legacy cancelled row stored as non-recoverable normalizes to trusted and eligible', async () => {
  const workspace = root();
  const h = harness();
  await seedPending(h.journal, 'agentcmd_legacy_cancel', workspace);
  await cancelRun(h.journal, 'agentcmd_legacy_cancel');
  ...
  const summary = (await summaryOf(h.history, 'agentcmd_legacy_cancel', workspace));
  assert.equal(summary.integrity, 'TRUSTED');
...
test('history list and detail report identical integrity and recovery classification', async () => {
  ...
  await seedPending(h.journal, 'agentcmd_a', workspace); rejectRun(h.journal, 'agentcmd_a');
  await seedPending(h.journal, 'agentcmd_b', workspace); cancelRun(h.journal, 'agentcmd_b');
  await seedPending(h.journal, 'agentcmd_c', workspace); expireRun(h.journal, 'agentcmd_c');
  await seedPending(h.journal, 'agentcmd_d', workspace); completeRun(h.journal, h.persistence, 'agentcmd_d');
  await h.service.reproposeFromRun('agentcmd_a', { requestId: '55555555-5555-4555-8555-555555555555' }, ctx);

  const list = (await h.history.list({ limit: 50, sort: 'requestedAt.desc' }, ctx));
  assert.equal(list.ok, true);
  if (!list.ok) return;
  assert.ok(list.value.runs.length >= 4);
  for (const row of list.value.runs) {
    const summary = (await summaryOf(h.history, row.runId, workspace));
...
test('no history surface can approve, resume, or execute a source or successor', async () => {
  ...
  const sourceFingerprint = (await h.journal.loadRun('agentcmd_src'))!.proposalFingerprint;
  ...
  const successorFingerprint = (await h.journal.loadRun(successorId))!.proposalFingerprint;
