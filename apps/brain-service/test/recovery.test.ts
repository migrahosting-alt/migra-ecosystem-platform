...
test('recovery plan is generated with ZERO writes (workspace untouched during planning)', async () => {
  ...
  const plan = await rec.plan('corr_orig');
...
test('recovery requires EXACT single-use approval; mismatch + replay refused', async () => {
  const { root, reverse } = mixedWorkspace();
  const { rec } = managers();
  rec.stashReverseMaterial('corr_orig', root, reverse, 'inc1');
  const plan = await rec.plan('corr_orig');
  await assert.rejects(async () => rec.apply(plan.recoveryId, 'rec_WRONG', fs), (e: unknown) => e instanceof RecoveryError && e.code === 'APPROVAL_MISMATCH');
  // Exact token applies once.
  await rec.apply(plan.recoveryId, plan.approvalToken, fs);
  // Replay refused.
  await assert.rejects(async () => rec.apply(plan.recoveryId, plan.approvalToken, fs), (e: unknown) => e instanceof RecoveryError && e.code === 'APPROVAL_REPLAYED');
});

test('approved recovery restores the workspace (contained), then verify passes', async () => {
  const { root, reverse } = mixedWorkspace();
  const { rec } = managers();
  rec.stashReverseMaterial('corr_orig', root, reverse, 'inc1');
  const plan = await rec.plan('corr_orig');
  const res = await rec.apply(plan.recoveryId, plan.approvalToken, fs);
  ...
  const evidence = await rec.verify(plan.recoveryId, fs);
  assert.equal(evidence.ok, true);
  assert.ok(evidence.checked >= 2);
});

test('simulate previews recovery with zero writes', async () => {
  const { root, reverse } = mixedWorkspace();
  const { rec } = managers();
  rec.stashReverseMaterial('corr_orig', root, reverse, 'inc1');
  const plan = await rec.plan('corr_orig');
...
test('incident CANNOT resolve without passing validation evidence', async () => {
  const { root, reverse } = mixedWorkspace();
  ...
  await rec.apply(plan.recoveryId, plan.approvalToken, fs);
  // No evidence → refused.
  await assert.rejects(async () => rec.resolve(plan.recoveryId, { ok: false, checked: 0 }), (e: unknown) => e instanceof RecoveryError && e.code === 'NO_VALIDATION_EVIDENCE');
  ...
  const evidence = await rec.verify(plan.recoveryId, fs);
  await rec.resolve(plan.recoveryId, evidence);
...
test('recovery audit chain links to the original incident + execution, no content leaked', async () => {
  const { root, reverse } = mixedWorkspace();
  const { rec, incidentId } = managers();
  rec.stashReverseMaterial('corr_orig', root, reverse, incidentId);
  const plan = await rec.plan('corr_orig');
  await rec.apply(plan.recoveryId, plan.approvalToken, fs);
  await rec.resolve(plan.recoveryId, await rec.verify(plan.recoveryId, fs));
...
test('recovery tolerates reverse material for a create whose write never landed (delete of an absent file)', async () => {
  ...
  assert.equal((await rec.verify(plan.recoveryId, fs)).ok, true);
});

test('resolveWithEvidence throws without evidence (incident manager guard)', async () => {
  const sink = new LocalAlertSink();
  const inc = new IncidentManager(sink.sink, () => 1);
