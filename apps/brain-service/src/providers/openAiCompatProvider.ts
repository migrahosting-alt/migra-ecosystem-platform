  ...
  private buildMessages(
    ...
    parts.push({
      ...
      content:
        ...
        // CRITICAL honesty rule. Asked to "report repository path, HEAD, commands
        // executed and exact results", the model invented an entire completion
        // report: a repo at /home/user/migrawatch (a path that does not exist), a
        // placeholder HEAD sha, 30+ files, and command output ("Done in 3.5s",
        // "Build successful with 0 warnings") for work that never ran.
        'YOU DO NOT EXECUTE ANYTHING in this conversation: you cannot create, edit or delete files, cannot run commands, and cannot change git state. NEVER state or imply that you created files, ran a command, installed anything, initialised a repository, or produced git output. NEVER invent file listings, directory trees, command output, test/build results, timings, commit SHAs, branch names, remotes or paths. If the user asks you to perform work or to report execution results, say plainly that nothing was executed here and either describe the plan or tell them to run the build task — a fabricated report is far worse than saying "I did not run this". Report ONLY facts present in the attached context. ' +
