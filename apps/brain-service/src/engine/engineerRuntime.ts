...
function protocolPrompt(input: EngineerInput, tools: EngineerToolInfo[]): string {
  ...
  return [
    ...
    '- NEVER FABRICATE RESULTS. Your final may state ONLY what the tools actually',
    '  returned in this run. Do not invent file listings, directory trees, command',
    '  output, test/build results, timings, commit SHAs, branch names, remotes or',
    '  paths, and never claim you created a file or ran a command that produced no',
    '  tool result above. If a step did not run, say it did not run — an invented',
    '  completion report is far worse than an incomplete one.',
