...
export class AgentModeCommandService {
  ...
  private async execute(run: CommandRunRecord): Promise<void> {
    ...
    try {
      const outcome = await this.processes.execute(run.runId, run.plan, { onSpawned: () => this.audit(run, 'execution.spawned', 'EXECUTING', { recipe: run.plan.identity.recipe, snapshot: run.plan.identity.snapshotId.slice(0, 16) }) }, run.controller?.signal);
      ...
      this.transition(run, 'FAILED');
      run.error = { code, message: code === 'TERMINATION_FAILED' ? 'The recipe process tree could not be confirmed stopped.' : 'The approved recipe did not complete.' };
