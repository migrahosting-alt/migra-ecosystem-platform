...
export class EscalationController {
  ...
  async offer(input: OfferInput): Promise<OfferResult> {
    ...
    if (!decision.offered || !decision.reason || !decision.target) {
      await auditStore.append({ correlationId: input.correlationId, type: 'escalation.denied', component: 'escalation', outcome: reason ?? 'none', fields: { policy: input.policy, denied: decision.deniedReason ?? 'not offered' } });
    ...
    if (this.budget.isEnabled() && (estimate.costUnavailable || !isTrustedForHardEnforcement(estimate.pricingStatus))) {
      await auditStore.append({ correlationId: input.correlationId, type: 'budget.pricing_unknown', component: 'budget', fields: { provider: decision.target.providerId, model: decision.target.modelId } });
      await auditStore.append({ correlationId: input.correlationId, type: 'escalation.denied', component: 'escalation', outcome: reason ?? 'none', fields: { denied: 'pricing not trustworthy under hard enforcement' } });
    ...
    if (!pf.affordable) {
      await auditStore.append({ correlationId: input.correlationId, type: 'escalation.denied', component: 'escalation', outcome: reason ?? 'none', fields: { denied: pf.code ?? 'over budget' } });
    ...
    await auditStore.append({ correlationId: input.correlationId, type: 'escalation.offered', component: 'escalation', outcome: decision.reason, fields: { policy: input.policy, provider: decision.target.providerId, model: decision.target.modelId, estCostUsd: estimate.estimatedCostUsd, worstCaseUsd: estimate.worstCaseCostUsd } });
    return {
      offered: true,
  ...
  async approve(input: ApproveInput): Promise<ApproveResult> {
    ...
    if (!consumed.ok) {
      await auditStore.append({ correlationId: input.correlationId, type: 'escalation.denied', component: 'escalation', outcome: consumed.reason, fields: { stage: 'approve' } });
    ...
    if (!provider) {
      await auditStore.append({ correlationId: input.correlationId, type: 'escalation.denied', component: 'escalation', outcome: 'UNKNOWN_TARGET', fields: { provider: offer.target.providerId } });
    ...
    if (provider.kind !== 'cloud' || !provider.enabled || (provider.credentialEnv && !this.providers.hasCredential(provider))) {
      await auditStore.append({ correlationId: input.correlationId, type: 'escalation.denied', component: 'escalation', outcome: 'TARGET_INELIGIBLE', fields: { provider: provider.id } });
    ...
    if (estimate.worstCaseCostUsd > offer.costCeilingUsd) {
      await auditStore.append({ correlationId: input.correlationId, type: 'escalation.denied', component: 'escalation', outcome: 'CEILING_EXCEEDED', fields: { worstCaseUsd: estimate.worstCaseCostUsd, ceilingUsd: offer.costCeilingUsd } });
    ...
    if (!rr.ok) {
      await auditStore.append({ correlationId: input.correlationId, type: 'escalation.denied', component: 'escalation', outcome: rr.code, fields: { stage: 'reserve' } });
      return { gate: true, code: rr.code, detail: rr.detail };
    }

    await auditStore.append({ correlationId: input.correlationId, type: 'escalation.approved', component: 'escalation', outcome: offer.reason, fields: { provider: provider.id, model: offer.target.modelId, reason: offer.reason, reservationId: rr.reservation.reservationId } });
