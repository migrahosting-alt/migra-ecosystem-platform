...
export const OPS_ACTION_REGISTRY: readonly OpsActionEntry[] = [
  ...
  },
  {
    ...
    riskLevel: "low",
    requiresApproval: true,
    description: "Transition an internal ops status marker (planned→in_progress→verifying→completed/failed/blocked…). Appends one internal journal record. Internal state only — NO infrastructure mutation. Invalid transitions are refused and write nothing.",
    expectedEffect: "Record an internal status transition only; no infrastructure mutation.",
    allowedTargets: ["*"],
    requiredEnv: [],
    prerequisites: ["An existing marker (or target + currentStatus), a valid nextStatus, and a reason."],
    hazards: [],
    verificationRecommendations: ["ops.status_marker.history", "ops.status_marker.verify"],
  },
  {
