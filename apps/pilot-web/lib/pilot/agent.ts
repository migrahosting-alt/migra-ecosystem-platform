...
const TOOLS_NOTE = [
  "",
  "Tools (use only when needed): git.status, git.log, git.diff, repo.search, repo.read_file, repo.list_files, image.info, image.analyze, image.health, image.preview, memory.search, memory.preview (read-only) and scratch.write_file, image.resize, image.convert, image.crop, image.annotate, image.generate, memory.ingest (these write/change something and need approval).",
