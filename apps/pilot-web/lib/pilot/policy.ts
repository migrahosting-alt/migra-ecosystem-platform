...
export function classifyPilotAction(name: string, args: Record<string, unknown> = {}): PilotActionDecision {
  ...
  if (name === "ops.promotion.evidence_export_preview") return mk("safe_read", "preview-only redacted export of the promotion evidence bundle (reuses the report-export engine); writes no file, executes nothing, fails closed on residual secrets");
