            ...
            <BrandDetail
              ...
              actions={{
                runDomainCheck: {
                  ...
                  onClick: async () => {
                    try {
                      const res = await fetch(`/api/brands/${selectedBrand.slug}/domain-check`, { method: "POST" });
                      const data = await res.json();
                      if (!data.ok) { setError(data.error ?? "Domain check failed"); return; }
                      void load();
                ...
                generateLaunchKit: {
                  ...
                  onClick: async () => {
                    try {
                      const res = await fetch(`/api/brands/${selectedBrand.slug}/launch-kit`, { method: "POST" });
                      const data = await res.json();
                      if (!data.ok) { setError(data.error ?? "Launch kit generation failed"); return; }
                      void load();
                    } catch (e) { setError(String(e)); }
                  },
                },
