...
interface AutonomyAction {
  ...
  type: string;
  targetSystem: string;
  executionStatus: "planned" | "gated" | "deferred" | "simulated";
  suggestedCommand?: string;
  risk: { level: "LOW" | "MEDIUM" | "HIGH" | "CRITICAL"; requiresOwnerApproval: boolean; reason: string };
}
...
const METRIC_TONE: Record<NonNullable<ExecutiveMetric["tone"]>, string> = {
  neutral: "var(--muted)",
  success: "var(--ok)",
  warning: "var(--warn)",
  danger: "var(--danger)",
...
export default function AutonomyPage() {
  ...
  async function refreshAll() {
    const [configRes, findingsRes, queueRes, statesRes, missionsRes, reportRes] = await Promise.all([
      ...
      fetch(pilotApiUrl("/api/autonomy/report"), { cache: "no-store" }),
    ]);
    ...
    const reportPayload = (await reportRes.json().catch(() => ({}))) as { ok: boolean; data?: ExecutiveReport };
    ...
    if (reportPayload.ok && reportPayload.data) setExecutiveReport(reportPayload.data);
  }
  ...
  async function executeRecommendedActions() {
    const response = await fetch(pilotApiUrl("/api/autonomy/execute"), {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ executeLowRisk: true })
    });
    const payload = (await response.json()) as {
      ok: boolean;
      data?: { executed: number; gated: number; failed: number };
      error?: string;
    };
    if (!payload.ok || !payload.data) {
      setMessage(payload.error ?? "Autonomy execute failed");
      return;
    }
    setMessage(`Executed ${payload.data.executed}, gated ${payload.data.gated}, failed ${payload.data.failed}`);
    await refreshAll();
  }
      ...
      {executiveReport ? (
        <div className="panel" style={{ padding: 12, marginBottom: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <div>
              <div style={{ fontWeight: 600 }}>MigraAutonomy Executive Report</div>
              <div className="small" style={{ color: "var(--muted)" }}>
                Generated {new Date(executiveReport.generatedAt).toLocaleString()}
              </div>
            </div>
            <div className="small" style={{ color: "var(--muted)" }}>
              {executiveReport.dashboard.topSignal ? `Top signal: ${executiveReport.dashboard.topSignal.type}` : "No dominant signal"}
            </div>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 10, marginBottom: 12 }}>
            {executiveReport.dashboard.metrics.map((metric) => (
              <div key={metric.label} className="panel" style={{ padding: 10 }}>
                <div className="small" style={{ color: "var(--muted)" }}>{metric.label}</div>
                <div style={{ fontSize: 22, fontWeight: 700, color: metric.tone ? METRIC_TONE[metric.tone] : "inherit" }}>{metric.value}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 12 }}>
            <div className="panel" style={{ padding: 10 }}>
              <div style={{ fontWeight: 600, marginBottom: 8 }}>Strategy Recommendations</div>
              <div style={{ display: "grid", gap: 8 }}>
                {executiveReport.strategy.slice(0, 3).map((item) => (
                  <div key={item.id} style={{ borderBottom: "1px solid var(--line)", paddingBottom: 8 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                      <div style={{ fontWeight: 600 }}>{item.title}</div>
                      <div className="small" style={{ color: "var(--muted)" }}>P{item.priority}</div>
                    </div>
                    <div className="small" style={{ color: "var(--muted)", marginTop: 4 }}>{item.summary}</div>
                    <div className="small" style={{ marginTop: 4 }}>{item.reason}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="panel" style={{ padding: 10 }}>
              <div style={{ fontWeight: 600, marginBottom: 8 }}>Recommended Commands</div>
              <div style={{ display: "grid", gap: 8 }}>
                {executiveReport.actions.slice(0, 4).map((action) => (
                  <div key={action.id} style={{ borderBottom: "1px solid var(--line)", paddingBottom: 8 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                      <div style={{ fontWeight: 600 }}>{action.type}</div>
                      <div className="small" style={{ color: action.risk.level === "LOW" ? "var(--ok)" : action.risk.level === "MEDIUM" ? "var(--warn)" : "var(--danger)" }}>
                        {action.risk.level}
                      </div>
                    </div>
                    <div className="small" style={{ color: "var(--muted)", marginTop: 4 }}>{action.targetSystem} · {action.executionStatus}</div>
                    {action.suggestedCommand ? <div style={{ fontFamily: "var(--mono)", fontSize: 11, marginTop: 4 }}>{action.suggestedCommand}</div> : null}
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="grid-2">
        <div className="panel" style={{ padding: 12 }}>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
            ...
            <button onClick={() => void executeRecommendedActions()}>Run Safe Actions</button>
