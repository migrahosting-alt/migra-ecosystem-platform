...
export interface StreamHandlers {
  ...
  /** Phase D: a live mutation is waiting for a human. Carries the EXACT action. */
  onApprovalRequest?: (card: ApprovalCardData) => void;
      ...
      for (;;) {
        ...
        while ((sep = buffer.indexOf("\n\n")) >= 0) {
          ...
          if (evName === "conversation" && typeof data.conversationId === "string") {
            ...
            /* Phase D: the approval CARD carries the exact action and the buttons. This step line
             * used to be the whole story — "enable live execution to allow", with nothing to
             * click and no way to proceed. Now it is just a pointer to the card below it. */
            if (appr) h.onStep(`🔐 ${appr.toolName ?? tool} needs your approval — see the card below`);
          ...
          /* Phase D — a live mutation is waiting for a human. This used to be a dead end: the
           * loop reported APPROVAL_REQUIRED, the extension printed "enable live execution to
           * allow", and there was nothing to click. Now the exact action arrives here and is
           * rendered as a card the operator can approve or reject. */
          else if (evName === "approval_request" && typeof data.pendingActionId === "string") {
            h.onApprovalRequest?.({
              pendingActionId: String(data.pendingActionId),
              approvalId: String(data.approvalId ?? ""),
              toolName: String(data.toolName ?? ""),
              summary: String(data.summary ?? ""),
              args: (data.args ?? {}) as Record<string, unknown>,
              mode: data.mode === "live" ? "live" : "dry_run",
              tenantScope: data.tenantScope ?? null,
