...
import { readModelImage } from '@/server/files/imageStore'
import { isImageId } from '@/server/files/images'

export { MODEL_IMAGE_MAX_EDGE } from '@/server/files/downscale'
 ...
 *
 * WHAT THE MODEL SEES IS NOT WHAT IS STORED. Large images are downscaled for the
 * model only — see `downscale.ts` for why that is the difference between a
 * two-second answer and a twenty-eight-second one. The library keeps the
 * original bytes and the transcript keeps showing the user's real picture.
 */
...
export interface TurnImageResolution {
  ...
  /** What downscaling actually did, for the turn's trace. */
  prepared: {
    ref: string
    storedBytes: number
    sentBytes: number
    downscaled: boolean
    /** False means this turn paid the decode cost rather than reading it warm. */
    fromCache: boolean
  }[]
}
...
export async function resolveTurnImages(refs: readonly string[]): Promise<TurnImageResolution> {
  ...
  const prepared: TurnImageResolution['prepared'] = []
  ...
  for (const ref of refs) {
    ...
    const found = await readModelImage(ref)
    if (!found) {
      /*
    ...
    prepared.push({
      ref,
      storedBytes: found.meta.bytes,
      sentBytes: found.bytes.byteLength,
      downscaled: found.downscaled,
      fromCache: found.fromCache,
    })
  }

  return { attachments, dropped, prepared }
}
