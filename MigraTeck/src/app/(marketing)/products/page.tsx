...
function buildMigraHostingPricing(): PricingTier[] {
  return MIGRAHOSTING_VPS_PLANS.map((plan) => ({
    name: plan.name,
    monthlyPrice: plan.monthlyPriceCents,
    interval: "month",
    features: plan.highlights,
    ...(plan.highlighted ? { highlighted: true } : {}),
  }));
}
      ...
      <section className="px-6 pb-16">
        <div className="mx-auto w-full max-w-7xl">
          ...
          {PRODUCT_CATALOG.map((product) => {
            const migraHostingPricing = product.key === "MIGRAHOSTING" ? buildMigraHostingPricing() : undefined;
