  ...
  return (
    <header className="site-header sticky top-0 z-50 px-4 pt-4 sm:px-6">
      <div className="mx-auto max-w-7xl">
        <nav
          className="site-header-shell relative overflow-hidden rounded-[28px] border border-white/10 bg-[#0B1220]/90 px-4 py-3 shadow-[0_24px_80px_rgba(8,15,36,0.35)] backdrop-blur-xl sm:px-5 sm:py-4 lg:px-6"
          role="navigation"
          aria-label="Main"
        >
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2 lg:hidden">
                <Link
                  ...
                  className="hidden rounded-full border border-sky-400/30 bg-sky-400/10 px-3 py-2 text-xs font-semibold text-sky-300 sm:inline-flex"
                >
                  Create account
                </Link>
                <button
            ...
            <div className="hidden items-center rounded-full border border-white/10 bg-white/5 px-2 py-1 lg:flex">
              {navigation.map((item) => (
          ...
          <div className="mt-3 flex gap-2 overflow-x-auto pb-1 lg:hidden">
            {navigation.slice(0, 5).map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="shrink-0 rounded-full border border-white/10 bg-white/5 px-3 py-2 text-xs font-semibold uppercase tracking-[0.14em] text-white/70 transition-colors hover:border-white/20 hover:text-white"
              >
                {item.label}
              </Link>
            ))}
            <button
              type="button"
              onClick={() => setMobileOpen((v) => !v)}
              className="shrink-0 rounded-full border border-white/10 bg-white/5 px-3 py-2 text-xs font-semibold uppercase tracking-[0.14em] text-white/70 hover:border-white/20 hover:text-white"
            >
        ...
        {mobileOpen && (
          <div className="site-header-panel mt-3 overflow-hidden rounded-[28px] border border-white/10 bg-[#0B1220]/95 p-6 shadow-[0_28px_90px_rgba(8,15,36,0.4)] backdrop-blur-xl lg:hidden">
            <div className="grid gap-3 sm:grid-cols-2">
              {navigation.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-semibold text-white/70 transition-colors hover:border-white/20 hover:bg-white/8 hover:text-white"
