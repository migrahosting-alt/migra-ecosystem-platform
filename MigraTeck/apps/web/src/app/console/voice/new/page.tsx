...
async function createVoiceNumber(formData: FormData) {
  ...
  //
  // This used to INSERT a provisioning_tasks row that nothing consumes, then redirect
  // to /console/voice as success. It also passed a freshly generated randomUUID() as
  // the "serviceInstanceId" — an id that referenced no service instance at all — so
  // even a working worker could not have resolved a target.
  //
  // Purchasing a phone number is a paid, irreversible upstream action. It must not be
  // faked. No number was ordered.
  redirect(
