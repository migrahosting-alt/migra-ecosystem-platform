# Apply Pack — NGINX TLS Hygiene + OCSP Cleanup + MigraPanel Rate Limits (Reload-only)

## Scope
Target host: srv1-web  
Goal: remove TLS protocol redefinition warnings, silence OCSP stapling ignored warnings safely, add MigraPanel rate limiting.  
...
2.4 MigraPanel limit zones (must be included in http{} context)
sudo tee /etc/nginx/snippets/migrapanel-limits.conf >/dev/null <<'EOF'
limit_req_zone $binary_remote_addr zone=migrapanel_auth:10m rate=5r/s;
limit_req_zone $binary_remote_addr zone=migrapanel_panel_api:10m rate=20r/s;
EOF

Step 3 — Ensure migrapanel-limits.conf is included in http{}

Edit /etc/nginx/nginx.conf and add inside http{}:

include /etc/nginx/snippets/migrapanel-limits.conf;
...
Step 5 — Add MigraPanel endpoint rate limits (migrapanel.com)

Inside migrapanel.com SSL server block:

Auth endpoints (strict)
location ~* ^/(api/auth|api/session) {
  limit_req zone=migrapanel_auth burst=10 nodelay;
  proxy_pass http://mpanel_backend;
}

General API (moderate)
location ^~ /api/ {
  limit_req zone=migrapanel_panel_api burst=40 nodelay;
  proxy_pass http://mpanel_backend;
}
...
curl -fsS https://control.migrahosting.com/ || true
...
.migra/runbooks/migrapanel-security.md (rate limits & rationale)
...
- `.migra/runbooks/migrapanel-security.md` (rate-limiting policy + tuning)
