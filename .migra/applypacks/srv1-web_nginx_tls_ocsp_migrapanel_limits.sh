...
log "Write /etc/nginx/snippets/migrapanel-limits.conf"
cat > /etc/nginx/snippets/migrapanel-limits.conf <<'EOF'
...
# Ensure migrapanel-limits is included inside http{} in nginx.conf (best-effort, safe)
NGINX_CONF="/etc/nginx/nginx.conf"
log "Ensure migrapanel-limits.conf included in http{} in $NGINX_CONF"
if ! grep -q "snippets/migrapanel-limits.conf" "$NGINX_CONF"; then
  # Insert after 'http {' line
  awk '
    BEGIN{done=0}
    {print}
    $0 ~ /^[[:space:]]*http[[:space:]]*\{/ && done==0 {
      print "    include /etc/nginx/snippets/migrapanel-limits.conf;";
      done=1
    }
  ' "$NGINX_CONF" > "${NGINX_CONF}.tmp"
  mv "${NGINX_CONF}.tmp" "$NGINX_CONF"
else
  log "migrapanel-limits.conf already included"
fi
